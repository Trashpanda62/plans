(function (global) {
  ...
  function applyCombo(combo) {
    Object.keys(combo).forEach(function (key) {
      var attribute = 'data-' + key.replace(/_/g, '-').replace(/[A-Z]/g, function (letter) { return '-' + letter.toLowerCase(); });
      if (combo[key] === '') document.documentElement.removeAttribute(attribute);
      else document.documentElement.setAttribute(attribute, combo[key]);
    });
  }
  ...
  function lockedOutput(combo) {
    var attributes = Object.keys(combo).map(function (key) {
      if (combo[key] === '') return '';
      return 'data-' + key.replace(/_/g, '-').replace(/[A-Z]/g, function (letter) { return '-' + letter.toLowerCase(); }) + '="' + combo[key].replace(/&/g, '&amp;').replace(/"/g, '&quot;') + '"';
    }).filter(Boolean).join(' ');
